...
UNIDOCK_PATH = os.path.join(root, "..", "processed", "unidock_REAL_docking_2")
...
# Sort by protein
df = pocket_detection_data.copy()
df["_r"] = df.groupby("Uniprot AC").cumcount()
df = df.sort_values(["_r", "Uniprot AC"]).drop(columns="_r").reset_index(drop=True)
