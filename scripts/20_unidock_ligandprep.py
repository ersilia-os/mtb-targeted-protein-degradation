...
PATH_TO_CONFORMATIONS_ENAMINE = "../output/unidock_docking/conformations"
PATH_TO_CONFORMATIONS_PREPARED = "../output/unidock_docking/conformations_prepared"
...
with tarfile.open("../output/enamine_characterization/conformations.tar.gz", 'r') as tar:
...
with open("../output/unidock_docking/input_ligands.txt", "w") as outfile:
    for sdf in sorted(os.listdir(PATH_TO_CONFORMATIONS_ENAMINE)):
            outfile.write(os.path.join(".", "..", "output", "unidock_docking", "conformations", sdf))
            outfile.write("\n")
...
COMMAND = f"unidocktools ligandprep --ligand_index ../output/unidock_docking/input_ligands.txt --savedir ../output/unidock_docking/conformations_prepared --batch_size 100 --use_file_name"
os.system(COMMAND)
...
with open("../output/unidock_docking/input_ligands.txt", "w") as outfile:
    for sdf in sorted(os.listdir(PATH_TO_CONFORMATIONS_PREPARED)):
            outfile.write(os.path.join(".", "..", "output", "unidock_docking", "conformations_prepared", sdf))
            outfile.write("\n")
