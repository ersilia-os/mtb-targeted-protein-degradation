#!/usr/bin/env python3
"""
Reads script 86's affinity results CSV (one row per gene_name x compound) and joins in smiles
(from compounds.csv) plus protein_name/ec_number (from protein_sequences.csv -- useful here since
there are 38 genes rather than 5 to keep straight). Same design as the Mtb screen's script 82:
stays raw/lossless (one row per gene x compound result), plots a per-gene affinity_pred_value
distribution (IC50 uM) and affinity_pred_value vs affinity_probability_binary scatter with
Pearson/Spearman correlation.

Usage:
    python 88_nesso1_human_collect_affinities.py [--out-subdir nesso_results]
"""
import argparse
import os
import sys
...
OUTPUT_DIR = os.path.join(ROOT, "output", "88_nesso1_human_collect_affinities")
...
RESULT_COLUMNS = (
    ["gene_name", "uniprot_ac", "protein_name", "ec_number", "compound_id", "smiles"]
...
def parse_args():
    ...
    parser.add_argument("--out-subdir", default="nesso_results",
                         help="Output subdirectory script 86 wrote to, e.g. for a scoped/smoke-test "
...
def local_csv_path(out_subdir):
    ...
    if not os.path.isfile(path):
        sys.exit(f"No affinity results CSV found at {path}; has script 86 written it yet?")
    return path


def merge_results(raw_df, proteins, compounds):
    df = raw_df.merge(proteins[["gene_name", "uniprot_ac", "protein_name", "ec_number"]],
