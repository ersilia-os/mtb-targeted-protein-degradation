"""
Figure 5, panel a: PyMOL screenshot of PDB 7K98, the crystal structure of M. tuberculosis PheRS
...
Panel size comes from output/plots/figure_5/panel_layout.csv (columns: panel, x, delta_x, y,
delta_y, padding - x/y are each panel's top-left placement, used by merge_panels() (called
automatically at the end of main(); formerly a separate figure_5_merge.py script, now folded into
...
panel's content without affecting any font/tick/label size), same convention as figure_4_plot.py's
...
(output/plots/figure_5/compounds/ids.csv, column cpd_id holding selection_table.csv source_key
...
Usage:
    python figure_5_plot.py [--rerun] [--subpanels a,b,c,d,e]
"""
import argparse
import io
import json
...
plots_dir = os.path.join(ROOT, "output", "plots", "figure_5")
...
def load_panel_sizes(panels):
    """panel -> (width_in, height_in), from output/plots/figure_5/panel_layout.csv - same
    convention as figure_4_plot.py's own load_panel_sizes."""
...
def load_panel_padding(panels):
    ...
    figure_4_plot.py's own load_panel_padding."""
    df = pd.read_csv(panel_layout_path).set_index("panel")
...
def apply_padding(fig, padding):
    ...
    near-verbatim from figure_4_plot.py's own apply_padding."""
    scale = 1 - padding
    for ax in fig.axes:
        pos = ax.get_position()
...
PANEL_LABEL_MARGIN = 0.02  # figure-fraction inset from the page corner - ported from figure_4_plot.py
...
def add_panel_label(fig, letter, x_margin=PANEL_LABEL_MARGIN):
    ...
    figure_4_plot.py's own add_panel_label (already reused as-is by figure_1_plot.py/
    figure_2_plot.py; figure_5 was the only one of the four missing it). x_margin overrides the
...
def save_panel(fig, letter, use_tight_layout=True, tight_pad=1.08, tight_w_pad=None,
    ...
    # convention as figure_4_plot.py's save_panel. use_tight_layout=False (panel b's grid, and
    ...
    output_path = os.path.join(plots_dir, f"Fig_5{letter}.pdf")
    plt.savefig(output_path, dpi=600, transparent=False)
    plt.close(fig)
    print(f"Saved Fig_5{letter}.pdf")
...
def merge_panels():
    """Pastes each Fig_5{letter}.pdf onto one positioned master PDF (Fig_5_full.pdf) per
    ...
    figure_5_merge.py (now removed) so plot + merge happen in one script, matching
    figure_1_plot_v2.py/figure_2_plot_v2.py/figure_4_plot.py's own convention."""
    df = pd.read_csv(panel_layout_path).set_index("panel")
    ...
    for p in PANEL_LETTERS:
        panel_path = os.path.join(plots_dir, f"Fig_5{p}.pdf")
        if not os.path.exists(panel_path):
            print(f"  Skipping {p}: {panel_path} not found - Fig_5_full.pdf will be missing it.")
            continue
        ...
        print(f"  Placed Fig_5{p}.pdf at x={x_top_cm}cm, y={y_top_cm}cm (top-left)")

    output_path = os.path.join(plots_dir, "Fig_5_full.pdf")
    ...
    png_path = os.path.join(plots_dir, "Fig_5_full.png")
...
def _color_ligand(selection, carbon_color, color_name):
    """Color-by-element with a custom carbon color - same helper as figure_4_plot.py's
...
PNG_PATH = os.path.join(renders_dir, "figure_5a_7K98.png")
PSE_PATH = os.path.join(renders_dir, "figure_5a_7K98.pse")
...
def render_7k98(rerun=False):
    ...
    # Standard project render settings, matching figure_1_plot.py / figure_4_plot.py /
...
# render_structure_panel (colored-circle-as-legend-handle trick) and figure_4_plot.py's
...
def _corner_badge(ax, dot_color, label, loc="upper left", anchor_point=(0, 1)):
    ...
    render_structure_panel / figure_4_plot.py's _corner_badge. anchor_point defaults to
...
def _site_type_label(ax, site_type):
    ...
    not flush against it), white alpha-blended box - same text-box style as figure_4_plot.py's
...
# target-group badge).
# Orange - the project's standard docked-pose color, verbatim from scripts/47b/51/figure_4_plot.py.
COLOR_LIGAND_DOCKED = [0xF5 / 255, 0xA6 / 255, 0x3A / 255]

# Same fixed Angstrom cube (centered on the ligand) as figure_4_plot.py's SHOWCASE_ZOOM_BOX, so
# every pocket snapshot in the grid is framed at the same physical scale regardless of protein size.
# 16.0 (figure_4's own value) put the camera essentially inside the pocket cavity here - close
...
def _extract_ligand_sdf(docking_dir, compound_id):
    ...
    returning its path (caller must remove it) - same tarfile idiom as figure_4_plot.py's
...
def render_pocket_snapshot(gene_name, site_type, pocket_name, rerun=False):
    ...
    png_path = os.path.join(renders_dir, f"figure_5b_{pocket_name}.png")
    pse_path = os.path.join(renders_dir, f"figure_5b_{pocket_name}.pse")
    if os.path.exists(png_path) and not rerun:
        print(f"Reusing existing render: {png_path}")
        return png_path
...
def _zoom_to_fixed_box_b(selection, box_size, box_name="_zoom_box_b"):
    """Same fixed-cube zoom helper as figure_4_plot.py's _zoom_to_fixed_box."""
...
def _render_upset_figure(hit_sets, element_size, title, show_legend=True, legend_degrees=None, show_yaxis=True):
    ...
    # stylize(), scripts/plots/figure_5_plot.py isn't itself using stylia.create_figure()/stylize()
...
def plot_upset_panel(letter, size, padding=0.0):
    ...
    cat_png = os.path.join(renders_dir, "figure_5c_upset_CAT.png")
    noncat_png = os.path.join(renders_dir, "figure_5c_upset_NONCAT.png")
