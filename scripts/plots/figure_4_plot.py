"""
Figure 4: multi-target docking hits from figure_4_calculations.py.

Each named panel (a-d) is rendered into its OWN standalone figure and saved as its own PDF
(Fig_4a.pdf ... Fig_4d.pdf, see main()/save_panel()), rather than assembled into one composite
figure - panels are meant to be laid out externally (e.g. in a manuscript figure tool). Each
panel's figsize comes from output/plots/figure_4/panel_layout.csv, a user-authored CSV with columns
...
figure_4_calculations.py's SELECTED_SET_CUTOFF multi-target hit set (compounds already selected as
...
figure_4_calculations.py's figure_4_gene_summary_stats.json - see TIER_ROW_FIELDS for the
...
pocket (figure_4_calculations.py's load_pocket_catalytic_map, CATALYTIC_CONFIDENCE_MIN); gatA and
...
figure_4_gene_summary_stats.json, instead reusing plot_circos_overlap's own node_strength metric
...
(TIER_GRID_NA) if unassessed - see figure_4_calculations.py's EXPERIMENTAL_TRACTABILITY_OVERRIDES,
...
figure_4_calculations.py's TOP_AVG_SCORE_COMPOUND_IDS, the 2 compounds (of the cutoff-12
...
(figure_4_calculations.py's compute_top_avg_score_compounds_pockets - the same per-gene min-score
...
(Fig_4_full.pdf), called automatically at the end of main() - formerly a separate
figure_4_merge.py script, now folded into this same file.
...
def load_panel_sizes(panels):
    """panel -> (width_in, height_in), from output/plots/figure_4/panel_layout.csv (columns:
...
# figure_4_calculations.py's PANEL_A_CUTOFFS/SELECTED_SET_CUTOFF.
...
def plot_protein_hit_counts(ax):
    counts = pd.read_csv(os.path.join(data_dir, "figure_4_selected_set_protein_hit_counts.csv"))
...
def plot_circos_overlap(ax):
    hits_path = os.path.join(data_dir, f"figure_4_multi_target_hits_cutoff{abs(CIRCOS_CUTOFF)}.csv")
...
# Row label -> figure_4_gene_summary_stats.json field -> how to grade it. "continuous_low_better"
...
# has no field of its own (it isn't in figure_4_gene_summary_stats.json) - see
# _compute_cross_pharmacology_scores below. The 4 docking rows split each library's combined
# best-score row by catalytic vs non-catalytic pocket (figure_4_calculations.py's
...
def _compute_cross_pharmacology_scores(cutoff):
    ...
    hits_path = os.path.join(data_dir, f"figure_4_multi_target_hits_cutoff{abs(cutoff)}.csv")
    selected = pd.read_csv(hits_path)
    gene_cols = [c.removeprefix("score_") for c in selected.columns if c.startswith("score_")]
...
def plot_tier_grid_placeholder(ax):
    ...
    figure_4_calculations.py's figure_4_gene_summary_stats.json (plus Cross-pharm., see
    ...
    an explicit novelty=False (figure_4_calculations.py's NOVELTY_OVERRIDES, currently only
    ileS) - blank otherwise."""
    with open(os.path.join(root, "..", "..", "output", "plots", "figure_1", "color_mapping.json")) as f:
        genes = sorted(json.load(f)["gene_to_color"].keys())
    with open(os.path.join(data_dir, "figure_4_gene_summary_stats.json")) as f:
        stats = json.load(f)
    ...
    for _, field, kind in TIER_ROW_FIELDS:
        if kind == "p2rank_absolute":
            ...
            # this field is categorical (figure_4_calculations.py's EXPERIMENTAL_TRACTABILITY_
...
def render_showcase_pocket(row, rerun=False):
    ...
    png_path = os.path.join(renders_dir, f"figure_4_showcase_{row['compound_id']}_{row['gene']}.png")
...
def render_2d_structure(compound_id, smiles, rerun=False):
    ...
    png_path = os.path.join(renders_dir, f"figure_4_structure2d_{compound_id}.png")
    if os.path.exists(png_path) and not rerun:
    ...
    # (same convention as figure_5_plot.py's _mol_image/autocrop_to_content); a transparent
    # margin reads as black rather than white to that detection, breaking both. Same reasoning as
    # figure_5_plot.py's own _mol_image, which found no visible/measurable difference between a
...
def autocrop_to_content(img, padding_frac=0.05, background_frac=0.98):
    ...
    as figure_1_plot.py's/figure_5_plot.py's own autocrop_to_content (duplicated per-file, same
...
# (user request), styled the same way as figure_5 panel e's own numbered compound cards
# (figure_5_plot.py's MOL_IMAGE_FILL_FRAC/_mol_image_zoom/_label_xy_for_mol), ported here rather
...
def _pose_row_slot_size_in(fig_size, n_cols):
    ...
    1 x n_cols row - same analytical approach as figure_5_plot.py's _slot_size_in, adapted for a
...
def _struct_image_zoom(slot_w_in, slot_h_in, raster_w, raster_h):
    ...
    slot_w_in x slot_h_in slot - verbatim port of figure_5_plot.py's _mol_image_zoom."""
    return STRUCT_IMAGE_FILL_FRAC * min(slot_w_in * 72 / raster_w, slot_h_in * 72 / raster_h)


# Compound-number label style - ported from figure_5_plot.py's own LABEL_FONT/LABEL_REACH_*/
# LABEL_FOOTPRINT_MARGIN (user request: "bold Arial", same as figure_5 panel e's own compound
# numbering).
...
def _struct_text_half_size_in(s):
    ...
    of figure_5_plot.py's _text_half_size_in."""
    fig = plt.figure()
    fig.canvas.draw()
...
def _label_xy_for_struct(img, mol_zoom, slot_w_in, slot_h_in, label_text):
    ...
    centered structure - ported from figure_5_plot.py's _label_xy_for_mol, dropping that
...
# hand-tuning convention as figure_1's MY_VIEWS / figure_5's VIEWS_DIR hand-tuned camera views.
STRUCT_LABEL_NUDGE = {
    1: (0.06, -0.06),
    2: (-0.04, 0.12),
}
...
def _draw_compound_pose_row(fig, row_subgs, compound_rank, rerun=False):
    ...
    pockets = pd.read_csv(os.path.join(data_dir, "figure_4_top_avg_score_compounds_pockets.csv"),
                           keep_default_na=False)
    # No .sort_values("gene") (alphabetical) - the CSV's own row order already IS the intended
    # display order (figure_4_calculations.py's SHOWCASE_GENES: pheS, aspS, lysS, alaS, per
    ...
    # OffsetImage+ink-avoiding-label mechanism as figure_5 panel e's own numbered compound cards
    # (user request).
...
def main(rerun=False, subpanels=None):
    ...
    # the user-authored output/plots/figure_4/panel_layout.csv, not a shared mosaic. subpanels
