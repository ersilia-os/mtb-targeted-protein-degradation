"""
Computes the data feeding figure_4_plot.py: multi-target docking hits from the Enamine REAL 10B
...
each of the 21 genes - a library-wide reference stat, not itself plotted by figure_4_plot.py.
...
loosest to strictest). Feeds figure_4_plot.py's panel a bars - this matches
...
Usage:
    python figure_4_calculations.py
"""
import os
import sys
...
# Loosest -> strictest; the population figure_4_plot.py's panel a breaks down by protein is the
...
def compute_multi_target_hits(gene_scores):
    ...
    for cutoff in CUTOFFS:
        ...
        out_path = os.path.join(data_dir, f"figure_4_multi_target_hits_cutoff{abs(cutoff)}.csv")
...
def compute_protein_hit_counts(gene_scores):
    ...
    out_path = os.path.join(data_dir, "figure_4_protein_hit_counts.csv")
...
def compute_gene_summary_stats(gene_scores, pocket_scores):
    ...
    # figure_3_calculations.py's own "HL" naming for the same library.
    ...
    # SELECTED_SET_CUTOFF - i.e. figure_4_plot.py's plot_circos_overlap own node_strength (row sum
    ...
    # SELECTED_SET_CUTOFF must stay equal to figure_4_plot.py's CIRCOS_CUTOFF for these to match.
    # Requires compute_multi_target_hits to have already written this cutoff's hits CSV.
    hits_path = os.path.join(data_dir, f"figure_4_multi_target_hits_cutoff{abs(SELECTED_SET_CUTOFF)}.csv")
    ...
    out_path = os.path.join(data_dir, "figure_4_gene_summary_stats.json")
...
def load_pocket_catalytic_map():
    ...
    rows by catalytic vs non-catalytic pocket (see figure_4_plot.py's TIER_ROW_FIELDS)."""
...
# genes there) - user-confirmed via AskUserQuestion, for figure_4_plot.py's panel d (one row per
...
# each compound's own cutoff-12 hit set (pheS, not pheT, since figure_4 keeps them as separate
# genes rather than figure_5's own merged "pheST" label). _compute_pockets_for_compound looks up
...
def compute_top_avg_score_compounds_pockets():
    ...
    out_path = os.path.join(data_dir, "figure_4_top_avg_score_compounds_pockets.csv")
    out.to_csv(out_path, index=False)
...
def compute_selected_set_protein_hits():
    ...
    out_path = os.path.join(data_dir, "figure_4_selected_set_protein_hit_counts.csv")
    counts.to_csv(out_path, index=False)
    print(counts.to_string(index=False))
    print(f"Saved to {out_path}")
